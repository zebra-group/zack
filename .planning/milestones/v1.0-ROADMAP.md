...
- [x] **Phase 10: OIDC/SSO Integration** - Optional SSO login, additive on top of magic-link auth, with new SSO users safely defaulting to Member with zero domains (completed 2026-07-23)
...
- [x] 10-01-PLAN.md — SSO ENV config (optional, all-three-or-none boot guard) + ssoConfig reader + SsoStatusDTO (AUTH-05, D-10-02/06/07)
- [x] 10-02-PLAN.md — Conditional genericOAuth in auth.ts + magic-link-unchanged & least-privilege safety tests (AUTH-06/07, D-10-01/03/04/05)
- [x] 10-03-PLAN.md — Read-only GET /api/sso/status endpoint (masked id, real callback, no secret) (AUTH-05, D-10-02/06)
- [x] 10-04-PLAN.md — TeamView "Authentifizierung" section + getSsoStatus client + /login preview guard (AUTH-05, UI-10-01..06/09)
- [x] 10-05-PLAN.md — LoginView conditional "Mit SSO anmelden" affordance (fail-closed, genericOAuth redirect) (AUTH-06, UI-10-07/08/10)
...
| 10. OIDC/SSO Integration | 5/5 | Complete    | 2026-07-23 |
