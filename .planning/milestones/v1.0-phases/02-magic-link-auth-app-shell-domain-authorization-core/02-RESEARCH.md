...
The three open questions raised above are formally closed for planning. Each resolution is bound to a specific plan/task that carries the verification.

| # | Open Question | Resolution | Bound to | Verification |
|---|---------------|------------|----------|--------------|
| OQ-1 | Does `signIn.magicLink` short-circuit before `sendMagicLink` for a never-seen email under `disableSignUp: true` (would break D-01's neutral response)? | RESOLVED. The allowlist gate lives INSIDE `sendMagicLink` (returns silently for non-allowlisted emails), and the neutral response is proven, not assumed: plan 02-04 Task 3 includes a P0 canary asserting the magic-link request returns a **byte-identical** status + body for an allowlisted vs. a never-seen email, with mail sent only for the allowlisted one. If the empirical response ever differs, 02-04 must add a response-normalizing wrapper before the phase is considered done. | 02-02 (in-callback gate) + 02-04 Task 3 (canary) | `pnpm --filter @kurzly/api test -- --run auth.integration.test.ts` |
| OQ-2 | Is the standalone `@better-auth/prisma-adapter` npm package now preferred over the bundled `better-auth/adapters/prisma` import? | RESOLVED. Use the **bundled `better-auth/adapters/prisma`** import per CLAUDE.md's locked guidance; the standalone package is explicitly NOT installed (02-01 acceptance criterion asserts its absence from package.json). A 2-minute live re-check of the current `better-auth.com/docs/adapters/prisma` install instructions is retained as an execution-time step in 02-02; if the docs now mandate the standalone package, the executor flags the CLAUDE.md discrepancy back to the user rather than silently deviating. | 02-01 (exclusion) + 02-02 Task 1 (bundled import) | `grep -L "@better-auth/prisma-adapter" apps/api/package.json` |
| OQ-3 | How is a non-domain-scoped "global admin" (who manages the allowlist) modeled given D-02's role model is domain-scoped? | RESOLVED. Phase 2 keeps it minimal: `seedInitialAdmin` upserts a plain `User` row (emailVerified) for `INITIAL_ADMIN_EMAIL` — sufficient for AUTH-01..04 and the invite-only allowlist — with **no schema-level global-admin flag** added this phase (D-02b keeps the schema minimal). The full global-admin/allowlist-management model is deferred to Phase 9 (Team Management). 02-04 Task 2 surfaces the decision in its SUMMARY if the executor believes a flag is unavoidable. | 02-02 (schema minimal) + 02-04 Task 2 (seed, no flag) | `apps/api/test/auth.integration.test.ts` (seeded-admin login round-trip) |

**Certification:** All three open questions are RESOLVED for the purpose of planning. OQ-1 and OQ-2 carry residual execution-time verification steps (the byte-identical canary and the live-docs re-check) that are explicitly bound to plan tasks above; neither blocks planning.
