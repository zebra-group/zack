...
Because the `zebra-group/zack` repository is **PRIVATE**, the published
image inherits that same PRIVATE visibility on GHCR. Pulling from
outside the workflow (e.g. onto a production host) still requires
...
This `read:packages` PAT is a **pull-time-only** credential for
authenticating an out-of-band `docker pull` — it is unrelated to the
release process itself, which (as above) needs no PAT at all. The CI
job's own `GITHUB_TOKEN` is only valid inside that workflow run and
cannot be reused for an out-of-band pull.
