...
beforeEach(async () => {
  ...
  // fixture tests per-domain scoping ("lists only the caller's domain-
  // scoped QRs"), not the D-09-02 account-admin bypass, and must default to
  // `accountRole: "member"` (schema default) so it never sees another
  // domain's QR codes.
  await prisma.user.upsert({
    where: { email: ROUTE_OWNER_EMAIL },
    update: { emailVerified: true },
    create: {
      id: "u_qr_route_owner",
      name: "QR Route Owner",
      email: ROUTE_OWNER_EMAIL,
      emailVerified: true,
    },
  });
  await prisma.user.upsert({
